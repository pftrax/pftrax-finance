...
import { Text, Flex, Box, Radio, Skeleton, LinkExternal, ArrowForwardIcon, ArrowBackIcon, useMatchBreakpoints } from '@rimauswap-libs/uikit'
...
const TransactionTable: React.FC<{
  ...
  const sortedTransactions = useMemo(() => {
    ...
    return transactions
      ...
      : []
  }, [transactions, page, sortField, sortDirection, txFilter, isXs, isSm])
           ...
           sortedTransactions.length > 0 ? (
             <>
               {sortedTransactions.map((data) => {
                 if (data) {
                  const abs0 = Math.abs(data.amountToken0)
                  const abs1 = Math.abs(data.amountToken1)
                  const outputTokenSymbol = data.amountToken0 < 0 ? data.token0Symbol : data.token1Symbol
                  const inputTokenSymbol = data.amountToken1 < 0 ? data.token0Symbol : data.token1Symbol                
                   return (
                      <FCard key={data.amountToken0+data.amountToken1}>
                          <Flex mb="20px" justifyContent="space-between" alignItems="center" style={{textAlign:'left'}}>
                            <Text  style={{ flex: 1, alignItems: 'center' }}>{t('Action')}:</Text>
                            
                              <Text ml="8px" style={{ flex: 1, alignItems: 'center' }}>
                                <LinkExternal href={getBscScanLink(data.hash, 'transaction')}>
                                    {data.type === TransactionType.MINT
                                      ? t('Add %token0% and %token1%', { token0: data.token0Symbol, token1: data.token1Symbol })
                                      : data.type === TransactionType.SWAP
                                      ? t('Swap %token0% for %token1%', { token0: inputTokenSymbol, token1: outputTokenSymbol })
                                      : t('Remove %token0% and %token1%', { token0: data.token0Symbol, token1: data.token1Symbol })}
                                  </LinkExternal>
                              </Text>
                              
                          </Flex>
                          <Flex mb="20px" justifyContent="space-between" alignItems="center" style={{textAlign:'left'}}>
                              <Text style={{ flex: 1, alignItems: 'center' }}>{t('Total Value')}:</Text>
                              <Text style={{ flex: 1, alignItems: 'center' }}>
                              ${formatAmount(data.amountUSD)}
                              </Text>
                          </Flex>
                          <Flex mb="20px" justifyContent="space-between" alignItems="center" style={{textAlign:'left'}}>
                              <Text style={{ flex: 1, alignItems: 'center' }}>{t('Token Amount')}:</Text>
                              <Text style={{ flex: 1, alignItems: 'center' }}>
                              {`${formatAmount(abs0)} ${data.token0Symbol}`}
                              </Text>
                          </Flex>
                          <Flex mb="20px" justifyContent="space-between" alignItems="center" style={{textAlign:'left'}}>
                              <Text style={{ flex: 1, alignItems: 'center' }}>{t('Token Amount')}:</Text>
                              <Text style={{ flex: 1, alignItems: 'center' }}>
                              {`${formatAmount(abs1)} ${data.token1Symbol}`}
                              </Text>
                          </Flex>
                          <Flex mb="20px" justifyContent="space-between" alignItems="center" style={{textAlign:'left'}}>
                              <Text style={{ flex: 1, alignItems: 'center' }}>{t('Account')}:</Text>
                              <Text style={{ flex: 1, alignItems: 'center' }}>
                                  <LinkExternal href={getBscScanLink(data.sender, 'address')}>
                                    {shortenAddress(data.sender)}
                                  </LinkExternal>
                                </Text>
                          </Flex>
                          <Flex mb="20px" justifyContent="space-between" alignItems="center" style={{textAlign:'left'}}>
                              <Text style={{ flex: 1, alignItems: 'center' }}>{t('Time')}:</Text>
                              <Text style={{ flex: 1, alignItems: 'center' }}>
                                {formatDistanceToNowStrict(parseInt(data.timestamp, 10) * 1000)}
                              </Text>
                          </Flex>
                    </FCard>
                   )
