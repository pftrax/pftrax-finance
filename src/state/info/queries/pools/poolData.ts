...
interface FormattedPoolFields
  extends Omit<PoolFields, 'volumeUSD' | 'reserveUSD' | 'reserveBNB' | 'reserve0' | 'reserve1' | 'token0Price' | 'token1Price'> {
  volumeUSD: number
  reserveUSD: number
  reserveBNB?: number
  reserve0: number
...
const parsePoolData = (pairs?: PoolFields[]) => {
  ...
  return pairs.reduce((accum: { [address: string]: FormattedPoolFields }, poolData) => {
    const { volumeUSD, reserveUSD, reserveBNB, reserve0, reserve1, token0Price, token1Price } = poolData
    accum[poolData.id] = {
      ...poolData,
    ...
    const fetch = async () => {
      ...
      if (error) {
        ...
        const formatted = poolAddresses.reduce((accum: { [address: string]: PoolData }, address) => {
          ...
          // console.log("current?.volumeUSD ", current?.volumeUSD)
          // console.log("oneDay?.volumeUSD ", oneDay?.volumeUSD)
          // console.log("twoDays?.volumeUSD ", twoDays?.volumeUSD)
          ...
          const reservePrice = current ? current.reserveBNB : 0
          const liquidityToken0 = current ? current.reserve0 : 0
          const liquidityToken1 = current ? current.reserve1 : 0

          // console.log("volumeUSD", volumeUSD, volumeUSDWeek, liquidityUSD)
