...
const useFetchedTokenDatas = (tokenAddresses: string[]): TokenDatas => {
    ...
    const fetch = async () => {
      ...
      if (error) {
        ...
        const formatted = tokenAddresses.reduce((accum: { [address: string]: TokenData }, address) => {
          ...
          // console.log("current = =", current, oneDay);
          ...
          // console.log("address = = ", volumeUSD, volumeUSDChange);
          ...
          // console.log("priceUSDChange", priceUSD, priceUSDOneDay)
          ...
          // const todayValume = current.tokenDayData.find((item) => {
          //   return item.date === Math.max(...current.tokenDayData.map(x => x.date))
          // })
          ...
          accum[address] = {
            ...
            volumeUSD,
            volumeUSDChange,
            derivedBNB: current ? current.derivedBNB : 0,
            totalLiquidity: current ? current.totalLiquidity :0,
            tokenDayData:0,
            volumeUSDWeek,
            txCount,
            liquidityUSD,
  ...
  }, [tokenAddresses, block24h, block48h, block7d, block14d, blockError, bnbPrices, t24h])
