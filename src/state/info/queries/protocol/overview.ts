...
const formatPancakeFactoryResponse = (rawPancakeFactory?: PancakeFactory) => {
  return {
    totalTransactions: rawPancakeFactory && rawPancakeFactory.totalTransactions? parseFloat(rawPancakeFactory.totalTransactions):0,
    totalVolumeUSD: rawPancakeFactory && rawPancakeFactory.totalVolumeUSD? parseFloat(rawPancakeFactory.totalVolumeUSD):0,
    totalLiquidityUSD: rawPancakeFactory && rawPancakeFactory.totalLiquidityUSD? parseFloat(rawPancakeFactory.totalLiquidityUSD):0,
  }
...
const useFetchProtocolData = (): ProtocolFetchState => {
  ...
  useEffect(() => {
    const fetch = async () => {
      ...
      // console.log(data?.pancakeFactories?.[0], data24?.pancakeFactories?.[0], data48?.pancakeFactories?.[0], "data?.pancakeFactories?.[0]")
      const overviewData = formatPancakeFactoryResponse(data?.rimauFactories?.[0])
      const overviewData24 = formatPancakeFactoryResponse(data24?.rimauFactories?.[0])
      const overviewData48 = formatPancakeFactoryResponse(data48?.rimauFactories?.[0])
      ...
      if (anyError || !allDataAvailable) {
        ...
        // console.log("overviewData.totalVolumeUSD,", overviewData.totalVolumeUSD)
        // console.log("overviewData24.overviewData24,", overviewData24.totalVolumeUSD)
        // console.log("overviewData48.overviewData48,", overviewData48.totalVolumeUSD)
